...
However some properties might failed to load e.g. because of a network error. Those will not be set on `state` and will be `undefined` if not set via `getInitialState` e.g. to an empty array.
